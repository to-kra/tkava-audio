...
class PlayStreamThread extends PlayThread {
	...
	public PlayStreamThread(AudioInputStream audioInputStream,
	...
	public void run() {
		try {
			...
			while (true) {
				...
				// the stopSound() method is called -- drain() is a blocking
				// method
...
/**
 * This class contains utility methods for loading and playing audio clips and
 * streams.
 * 
 * <p>
 ...
 * 
 * <p>
 * The various loading methods return <code>true</code> if the requested audio
 * was loaded successfully, and <code>false</code> otherwise. This value can be
 * used, for example, to display an error message:
 * 
 * <p>
 * <blockquote>
 * 
 ...
 * 
 * <p>
 * Then, play the sounds by specifying the names they were given, and whether or
 * not the sound should loop (<code>true</code> or <code>false</code>):
 * <p>
 * <blockquote>
 * 
 * <pre>
 ...
 * 
 * <p>
 * Sounds only need to be loaded once. Generally, if the sounds you want to load
 * are small and short, it is recommended that you use one of the
 * {@link #loadClip loadClip} methods. For longer sounds that may not fit in
 * memory, such as full songs, you should use one of the {@link #loadStream
 * loadStream} methods.
 * 
 * <p>
...
class AudioPlayer {
	...
	private static boolean loadClip(String soundName,
	...
	private static boolean loadStream(String soundName,
			AudioInputStream audioInputStream) {
		boolean retVal = true;
		...
		if (ad != null) {
			if ((ad.thread == null) || (!ad.thread.isAlive())) {
				if (ad.dataLine instanceof SourceDataLine) {
					PlayStreamThread pt = new PlayStreamThread(
							ad.audioInputStream, (SourceDataLine) ad.dataLine);
					ad.thread = pt;
